  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        head_row: "flex",
        head_cell:
          "text-zinc-500 rounded-md w-8 font-normal text-[0.8rem] dark:text-zinc-400 w-full",
        row: "flex w-full mt-2",
        cell: cn(
          "w-full relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected].day-range-end)]:rounded-r-md dark:[&:has([aria-selected])]:bg-zinc-800 dark:[&:has([aria-selected].day-outside)]:bg-zinc-800/50",
          props.mode === "range"
      ...
      components={{
        IconLeft: ({ className, ...props }) => (
          <PiCaretLeftBold
            className={cn("text-base text-orange", className)}
            {...props}
          />
        ),
